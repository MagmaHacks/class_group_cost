...
function class_group_expected_tries(d, D, bound)
  ...
  // so we cut off the sum at 15 (normally)
  ...
  function riemann_summands(n, l, u)
    ...
    for i := 1 to n do
      x *:= f;
      lx +:= lf;
      if lx gt 15 and sqrtD lt 10^100 then
         break;
      end if;
      V := hvol(d, x/sqrtD);
      v := V - V0;
...
{"} // "
  ...
  E := Ceiling(class_group_expected_tries(d, D, Bmin)); // MagmaHacks
  return Bmin, E; // MagmaHacks
